...
const TASK_DOC_MAPPING = {
  // 인증/권한 관련
  'auth': {
    docs: ['auth/permissions.md', 'auth/authentication.md', 'auth/security-policies.md'],
    apiDocs: true,
    examples: true
  },
...
async function analyzeCompletedTask(taskNumber) {
  ...
  const tasksContent = fs.readFileSync('.kiro/specs/realtime-collaborative-editor/tasks.md', 'utf8');
  
  // Task 내용 파싱
  const taskMatch = tasksContent.match(new RegExp(`- \\[x\\] ${taskNumber}\\. (.+?)(?=\\n- \\[|$)`, 's'));
...
function generateDocContent(taskInfo, docPath) {
  ...
  return template
    ...
    .replace(/{FEATURE_DESCRIPTION}/g, `Task ${taskInfo.number}에서 구현된 기능입니다.`)
