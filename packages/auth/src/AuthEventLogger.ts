...
export interface AuthEvent {
  type: 'LOGIN' | 'LOGOUT' | 'MFA_SETUP' | 'PASSWORD_RESET' | 'ACCOUNT_LOCKED' | 'SUSPICIOUS_ACTIVITY';
  userId: string;
  timestamp: Date;
  ip?: string;
  userAgent?: string;
...
export class AuthEventLogger {
   ...
   * 로그인 이벤트 로깅
   */
  async logLogin(userId: string, ip?: string, userAgent?: string, metadata?: Record<string, any>): Promise<void> {
    await this.logEvent({
      type: 'LOGIN',
      userId,
      ip,
   ...
   * 로그아웃 이벤트 로깅
   */
  async logLogout(userId: string, ip?: string, userAgent?: string, metadata?: Record<string, any>): Promise<void> {
    await this.logEvent({
      type: 'LOGOUT',
      userId,
      ip,
   ...
   * MFA 설정 이벤트 로깅
   */
  async logMfaSetup(userId: string, ip?: string, userAgent?: string, metadata?: Record<string, any>): Promise<void> {
    await this.logEvent({
      type: 'MFA_SETUP',
      userId,
      ip,
   ...
   * 비밀번호 재설정 이벤트 로깅
   */
  async logPasswordReset(userId: string, ip?: string, userAgent?: string, metadata?: Record<string, any>): Promise<void> {
    await this.logEvent({
      type: 'PASSWORD_RESET',
      userId,
      ip,
   ...
   * 계정 잠금 이벤트 로깅
   */
  async logAccountLocked(userId: string, ip?: string, userAgent?: string, metadata?: Record<string, any>): Promise<void> {
    await this.logEvent({
      type: 'ACCOUNT_LOCKED',
      userId,
      ip,
   ...
   * 의심스러운 활동 이벤트 로깅
   */
  async logSuspiciousActivity(userId: string, ip?: string, userAgent?: string, metadata?: Record<string, any>): Promise<void> {
    await this.logEvent({
   ...
   * 사용자별 최근 로그인 조회
   */
  async getRecentLogins(userId: string, limit: number = 10): Promise<AuthEvent[]> {
   ...
   * 보안 통계 생성
   */
  async getSecurityStats(userId?: string, days: number = 30): Promise<{
    totalLogins: number;
    uniqueIPs: number;
    ...
    const suspiciousActivities = events.filter(e => e.type === 'SUSPICIOUS_ACTIVITY');
   ...
   * 실시간 알림 처리
   */
  private async handleRealTimeNotification(event: Omit<AuthEvent, 'timestamp'>): Promise<void> {
    // 의심스러운 활동이나 계정 잠금 시 즉시 알림
    if (event.type === 'SUSPICIOUS_ACTIVITY' || event.type === 'ACCOUNT_LOCKED') {
