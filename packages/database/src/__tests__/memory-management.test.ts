  ...
  describe('Instance Management', () => {
    ...
    it('should track instance usage time', () => {
      ...
      setTimeout(() => {
        ...
        expect(stats2.instances[0].lastUsedAt).toBeGreaterThan(stats1.instances[0].lastUsedAt);
      }, 10);
    });
...
describe('Performance and Memory Efficiency', () => {
  ...
  it('should reuse existing instances when possible', () => {
    ...
    const stats = getMemoryStats();
    const currentWorkerInstances = stats.instances.filter(
      inst => inst.key.includes(process.env.JEST_WORKER_ID || 'main')
    );
