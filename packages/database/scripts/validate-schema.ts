...
class SchemaValidator {
  ...
  async validateAll(): Promise<void> {
      ...
      try {
        ...
        if (result.success) {
          ...
          if (result.details) {
            console.log(
              chalk.gray(
                `   세부사항: ${JSON.stringify(result.details, null, 2)}`
              )
            );
          }
          allPassed = false;
    ...
    if (allPassed) {
      ...
      console.log(
        chalk.red.bold('💥 일부 검증이 실패했습니다. 위의 오류를 확인해주세요.')
      );
      process.exit(1);
    }
  ...
  private async validateIndexes(): Promise<ValidationResult> {
    try {
      ...
      const indexes = await this.prisma.$queryRaw<
        Array<{ name: string; tbl_name: string }>
      >`
        SELECT name, tbl_name FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%';
      `;

      // 중요한 인덱스들이 있는지 확인
      const userEmailIndex = indexes.find(
        idx => idx.tbl_name === 'users' && idx.name.includes('email')
      );
  ...
  private async validateDataTypes(): Promise<ValidationResult> {
    try {
      ...
      const isBlobValid =
        page.document?.state instanceof Buffer ||
        page.document?.state instanceof Uint8Array;
